...
bool RestLogDestination::Init(){
	this->stop = false;
	std::thread *t = new std::thread(&domoaster::RestLogDestination::run, this);
	return true;
}
...
void RestLogDestination::Sending(Json::Value & message){
	...
	if(curl) {
		...
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, &domoaster::RestLogDestination::responseCallback);
